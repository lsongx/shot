...
# release
class SepDPerspTransDetector(nn.Module):
    def __init__(self, dataset, arch='resnet18', depth_scales=4):
        ...
        self.reducedgrid_shape_part = [self.reducedgrid_shape[0], self.reducedgrid_shape[1]//2]
        ...
        self.feat_down = nn.Sequential(
            # nn.Conv2d(out_channel, 128, 1), nn.LeakyReLU(0.1), 
            # # nn.Conv2d(128, 128, 3, padding=1), nn.LeakyReLU(0.1),
            # nn.Conv2d(128, 128, 3, padding=1)).to('cuda:0')
            ...
            # nn.Conv2d(out_channel, 128, 1, bias=False), 
            # nn.GroupNorm(16, 128), nn.ReLU(), 
            # nn.Conv2d(128, 128, 3, padding=1, bias=False),
            # nn.GroupNorm(16, 128), nn.ReLU()).to('cuda:0')
        out_channel = 128

        # assert out_channel%self.depth_scales==0
        # warp_channel = out_channel//self.depth_scales
        # self.feat_before_warp = nn.ModuleDict({
        #     f'{i}': nn.Conv2d(out_channel, warp_channel, 3, padding=1)
        #     for i in range(self.depth_scales)
        # }).to('cuda:0')
        # self.feat_before_merge = nn.ModuleDict({
        #     f'{i}': nn.Conv2d(warp_channel, out_channel, 3, padding=1)
        #     for i in range(self.depth_scales)
        # }).to('cuda:0')
        ...
        # uncertainty_channel = 8
        # self.uncertainty_before_merge = nn.Sequential(            
        #     nn.Conv2d(1, uncertainty_channel, 3, padding=1), nn.ReLU(),
        #     nn.Conv2d(uncertainty_channel, uncertainty_channel, 3, padding=1)).to('cuda:0')
        ...
        self.map_classifier = nn.Sequential(nn.Conv2d(out_channel*self.num_cam//2+2, 512, 3, padding=1), nn.ReLU(),
                                            ...
                                            # nn.Conv2d(128, 1, 1)).to('cuda:0')

                                            # nn.Conv2d(512, 128, 3, padding=1, bias=False), 
                                            # nn.GroupNorm(16, 128), nn.ReLU(),
                                            # nn.Conv2d(128, 128, 3, padding=1, bias=False), 
                                            # nn.GroupNorm(16, 128), nn.ReLU(),
                                            # nn.Conv2d(128, 1, 1, bias=False)).to('cuda:0')
        ...
        # self.uncertain_classifier = nn.Sequential(nn.Conv2d(out_channel, 64, 1), nn.ReLU(),
    ...
    # @torch.no_grad()
    # def get_uncertainty_score(self, depth):
    ...
    def warp_perspective(self, img_feature_all, part_idx):
        ...
        for i in range(self.depth_scales):
            ...
            out_feat = kornia.warp_perspective(in_feat, self.proj_mats[i][part_idx], self.reducedgrid_shape)
            warped_feat += self.feat_before_merge[f'{i}'](out_feat) # [b*n,c,h,w]
            # warped_feat += self.feat_before_merge[f'{i}'](
            #     torch.cat([out_feat, uncertainty], dim=1)) # [b*n,c,h,w]
        return warped_feat
        # uncertainty = self.uncertainty_before_merge(uncertainty)
        # return torch.cat([warped_feat, uncertainty], dim=1)
    ...
    def forward(self, imgs, imgs_gt=None, map_gt=None, alpha=0, visualize=False, part='left'):
        ...
        world_features = torch.cat([world_features.reshape([B,-1,*self.reducedgrid_shape_part]), self.coord_map.repeat([B, 1, 1, 1])], dim=1)
